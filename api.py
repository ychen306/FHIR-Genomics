...
def test():
	...
	for i in request.args.iteritems():
		print i
	from models import *	
	criteria = db.session.query(db.select([SearchParam.resource_id]).select_from(SearchParam).where(db.and_(SearchParam.name == "status", SearchParam.code == "final"))).subquery()
