...
def parse(datatype, data, correctible):
	...
	if datatype in SPECS:
		elements = [FHIRElement(element_spec, correctible) for element_spec in SPECS[datatype]['elements']]
		search_elements = [element.get_search_elements() for element in elements if element.validate(data)]
		if len(elements) != len(search_elements):
			return False, None
		search_elements = filter(lambda x: x.get('spec') is not None, search_elements)
...
def correct_element(element, element_types):
	for et in element_types:
		...
		elif et in FHIR_PRIMITIVE_VALIDATORS:
			validate_func = FHIR_PRIMITIVE_VALIDATORS[et]
			if validate_func(et):
				return str(et)
...
class FHIRElement(object):	
	...
	def __init__(self, spec, correctible):
		...
		if 'type' in spec['definition']:
			self.elem_types = [_type['code'] for _type in spec['definition']['type']]
	...
	def _push_ancestors(self, jsondict, path_elems, elem_ancestors):	
		...
		if isinstance(val, dict):
			...
			elem_ancestors.extend([(ancestor, path_elems[1:]) for ancestor in val])
	...
	def validate(self, data):
		...
		for parent in elem_parents:
			...
			if isinstance(elem, list):
				...
				# in this case, the elem itself is correct, with a cardinality or '*' but stored as a single item
